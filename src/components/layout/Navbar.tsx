import { Star, Check, Share2, CornerDownLeft } from "lucide-react";
...
export default function Navbar({ username, starsCount, onSearch }: NavbarProps) {
    ...
    <nav className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-sm">
      <div className="mx-auto flex max-w-5xl items-center justify-between px-4 sm:px-6 py-3 gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-3 font-mono text-sm sm:text-base tracking-tight shrink-0 flex-1 min-w-0">
          ...
          >
            <svg viewBox="10 51 100 100" className="h-5.5 w-5.5 sm:h-6.5 sm:w-6.5 fill-current">
          ...
          <form onSubmit={handleSearchSubmit} className="flex items-center gap-1 min-w-0 flex-1 md:max-w-50">
            ...
            <div className="relative flex-1 min-w-0">
              <input
                ...
                spellCheck={false}
                className="peer w-full bg-transparent border-b border-muted py-0.5 pr-5 text-foreground focus:outline-none focus:border-terminal-green transition-colors truncate tracking-wider font-bold text-xs sm:text-sm placeholder:text-muted-foreground/40"
              />
              {inputValue.trim().length > 0 && (
                <button
                  type="submit"
                  onMouseDown={(e) => e.preventDefault()}
                  className="absolute right-0 top-1/2 -translate-y-1/2 text-muted-foreground/80 hover:text-foreground transition-all duration-150 cursor-pointer opacity-0 pointer-events-none peer-focus:opacity-100 peer-focus:pointer-events-auto"
                  title="Submit"
        ...
        <div className="flex items-center gap-1 sm:gap-4 shrink-0">
          ...
          >
            ...
            <div className="hidden sm:flex border-l border-border bg-muted/50 px-2 py-1.5 font-bold text-foreground/80">
              {starsCount}
            </div>
          </a>
